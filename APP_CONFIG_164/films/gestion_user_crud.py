...
from APP_CONFIG_164.films.gestion_config_wtf_forms import FormWTFUpdateFilm, FormWTFAddFilm, FormWTFDeleteFilm
    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                return redirect(url_for('user_config_afficher', id_user_sel=0))
...
"""Editer(update) un film qui a été sélectionné dans le formulaire "user_config_afficher.html"
Auteur : OM 2022.04.11
...
def film_update_wtf():
    ...
    try:
        ...
        if form_update_film.validate_on_submit():
            ...
            return redirect(url_for('user_config_afficher', id_user_sel=id_film_update))
        elif request.method == "GET":
            # Opération sur la BD pour récupérer "id_user" et "config_use_case" de la "t_config"
            ...
            form_update_film.nom_film_update_wtf.data = data_user["user_firstname"]
            form_update_film.duree_film_update_wtf.data = data_user["user_lastname"]
            # Debug simple pour contrôler la valeur dans la console "run" de PyCharm
            print(f" duree film  ", data_user["user_lastname"], "  type ", type(data_user["user_lastname"]))
            form_update_film.description_film_update_wtf.data = data_user["user_birthdate"]
            form_update_film.cover_link_film_update_wtf.data = data_user["cover_link_film"]
            form_update_film.datesortie_film_update_wtf.data = data_user["date_sortie_film"]
...
"""Effacer(delete) un film qui a été sélectionné dans le formulaire "user_config_afficher.html"
Auteur : OM 2022.04.11
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_user_created_config WHERE fk_user = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_user WHERE id_user = %(value_id_film)s"""
            # Manière brutale d'effacer d'abord la "fk_user", même si elle n'existe pas dans la "t_user_created_config"
            # Ensuite on peut effacer le film vu qu'il n'est plus "lié" (INNODB) dans la "t_user_created_config"
            with DBconnection() as mconn_bd:
