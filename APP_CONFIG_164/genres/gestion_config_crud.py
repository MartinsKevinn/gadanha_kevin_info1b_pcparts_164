...
from APP_CONFIG_164.genres.gestion_config_wtf_forms import FormWTFAjouterGenres
from APP_CONFIG_164.genres.gestion_config_wtf_forms import FormWTFDeleteGenre
from APP_CONFIG_164.genres.gestion_config_wtf_forms import FormWTFUpdateGenre
...
"""
    ...
    Paramètres : order_by : ASC : Ascendant, DESC : Descendant
                id_config_sel = 0 >> tous les genres.
                id_config_sel = "n" affiche le genre dont l'id est "n"
"""
        ...
        try:
            with DBconnection() as mc_afficher:
                if order_by == "ASC" and id_config_sel == 0:
                    ...
                    strsql_config_afficher = """SELECT id_config, config_use_case, config_rating  FROM t_config WHERE id_config = %(value_id_config_selected)s"""
...
"""
    ...
    Remarque :  Dans le champ "name_genre_html" du formulaire "config/genres_ajouter.html",
    ...
    if request.method == "POST":
        try:
            if form.validate_on_submit():
                ...
                strsql_insert_genre = """INSERT INTO t_config (id_config,config_use_case) VALUES (NULL,%(value_intitule_genre)s) """
...
"""
    ...
    Remarque :  Dans le champ "nom_genre_update_wtf" du formulaire "config/config_update_wtf.html",
                le contrôle de la saisie s'effectue ici en Python.
                On transforme la saisie en minuscules.
                On ne doit pas accepter des valeurs vides, des valeurs avec des chiffres,
                des valeurs avec des caractères qui ne sont pas des lettres.
...
def config_update_wtf():
    ...
    try:
        ...
        if form_update.validate_on_submit():
            ...
            str_sql_update_intitulegenre = """UPDATE t_config SET config_use_case = %(value_name_genre)s, 
            config_rating = %(value_date_genre_essai)s WHERE id_config = %(value_id_genre)s """
            with DBconnection() as mconn_bd:
            ...
            return redirect(url_for('config_afficher', order_by="ASC", id_config_sel=id_genre_update))
        elif request.method == "GET":
            # Opération sur la BD pour récupérer "id_config" et "config_use_case" de la "t_config"
            str_sql_id_genre = "SELECT id_config, config_use_case, config_rating FROM t_config " \
                               "WHERE id_config = %(value_id_genre)s"
            valeur_select_dictionnaire = {"value_id_genre": id_genre_update}
            with DBconnection() as mybd_conn:
                mybd_conn.execute(str_sql_id_genre, valeur_select_dictionnaire)
            # Une seule valeur est suffisante "fetchone()", vu qu'il n'y a qu'un seul champ "nom genre" pour l'UPDATE
            data_nom_config = mybd_conn.fetchone()
            print("data_nom_config ", data_nom_config, " type ", type(data_nom_config), " genre ",
                  data_nom_config["config_use_case"])

            # Afficher la valeur sélectionnée dans les champs du formulaire "config_update_wtf.html"
            form_update.nom_genre_update_wtf.data = data_nom_config["config_use_case"]
            form_update.date_genre_wtf_essai.data = data_nom_config["config_rating"]
    ...
    try:
        ...
        if request.method == "POST" and form_delete.validate_on_submit():
            ...
            if form_delete.submit_btn_del.data:
                ...
                str_sql_delete_films_genre = """DELETE FROM t_user_created_config WHERE fk_config = %(value_id_genre)s"""
                str_sql_delete_idgenre = """DELETE FROM t_config WHERE id_config = %(value_id_genre)s"""
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT id_user_created_config, user_firstname, id_config, config_use_case FROM t_user_created_config 
                                            ...
                                            WHERE fk_config = %(value_id_genre)s"""
            ...
            with DBconnection() as mydb_conn:
                ...
                # Opération sur la BD pour récupérer "id_config" et "config_use_case" de la "t_config"
                str_sql_id_genre = "SELECT id_config, config_use_case FROM t_config WHERE id_config = %(value_id_genre)s"

                mydb_conn.execute(str_sql_id_genre, valeur_select_dictionnaire)
                # Une seule valeur est suffisante "fetchone()",
                # vu qu'il n'y a qu'un seul champ "nom genre" pour l'action DELETE
                data_nom_config = mydb_conn.fetchone()
                print("data_nom_config ", data_nom_config, " type ", type(data_nom_config), " genre ",
                      data_nom_config["config_use_case"])

            # Afficher la valeur sélectionnée dans le champ du formulaire "config_delete_wtf.html"
            form_delete.nom_config_delete_wtf.data = data_nom_config["config_use_case"]
